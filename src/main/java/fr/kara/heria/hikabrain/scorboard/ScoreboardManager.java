...
public class ScoreboardManager {
    ...
    public ScoreboardManager() {
        ...
        glowingTask = Hikabrain.getInstance().getScheduledExecutorService().scheduleAtFixedRate(() ->
        {
            String ip = colorIpAt();
            for (PersonalScoreboard scoreboard : scoreboards.values())
                Hikabrain.getInstance().getExecutorMonoThread().execute(() -> scoreboard.setLines(ip));
        }, 80, 80, TimeUnit.MILLISECONDS);

        reloadingTask = Hikabrain.getInstance().getScheduledExecutorService().scheduleAtFixedRate(() ->
        {
            for (PersonalScoreboard scoreboard : scoreboards.values())
                Hikabrain.getInstance().getExecutorMonoThread().execute(scoreboard::reloadData);
